...
# 2. Link auf der Hauptseite (index.md) wieder entfernen
root_index = Path('index.md')
if root_index.exists():
    root_content = root_index.read_text(encoding='utf-8')
    cleaned_content = re.sub(r'\n*---\n*\[⏱️ Chronologische Zeitachse aller Schicksale anzeigen\]\(zeitachse/\)', '', root_content)
    cleaned_content = re.sub(r'\[⏱️ Chronologische Zeitachse aller Schicksale anzeigen\]\(zeitachse/\)', '', cleaned_content)
    if cleaned_content != root_content:
        root_index.write_text(cleaned_content.strip() + "\n", encoding='utf-8')
        print("Link zur Zeitachse erfolgreich von der Hauptseite entfernt.")

print(f"Zeitachse im Hintergrund aktualisiert ({len(events)} Einträge).")
